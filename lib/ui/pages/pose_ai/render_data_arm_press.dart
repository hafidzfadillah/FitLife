...
class _RenderDataArmPressState extends State<RenderDataArmPress> {
  ...
  double? kneeLY;
  bool squatUp = false;
  String whatToDo = 'Mendeteksi postur...';
  ...
  Future<bool> _postureAccordingToExercise(
      Map<String, List<double>> poses) async {
    setState(() {
  ...
  _checkCorrectPosture(Map<String, List<double>> poses) async {
    if (await _postureAccordingToExercise(poses)) {
      if (!isCorrectPosture) {
        setState(() {
